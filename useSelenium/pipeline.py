...
class Pipeline(object):
    ...
    def __init__(self, HOST=setting.HOST):
    ...
    def pipe_mongo_check(self, data, **kw):
        ...
    ...
    def pipe_txt_load(self, **kw):
        ...
        """
        try:
            with open(self.path.format(kw.get('filename', 'default.txt')), kw.get('loadtype', 'r'),
                      encoding=kw.get('encoding', 'utf-8')) as f:
                return f.readlines()
        except:
            return []

    def pipe_pic_load(self, filename):
        """
        读取图片
        :return:
        """
        try:
            with open(self.path.format(filename), 'rb') as f:
                return f.read()
        except Exception as e:
            print('图片加载失败：{}'.format(e))
            return

    def pipe_pic_rename(self, filename, newname):
        """
        读取图片
        :return:
        """
        try:
            os.rename(self.path.format(filename), self.path.format(newname))
        except Exception as e:
            print(e)
            return

    def pipe_pic_del(self, filename):
        """
        删除图片
        :return:
        """
