...
import ControlNode.config as setting
...
class ControlManager(object):
    ...
    def manager_publish_task(self, start_url):
        ...
        task_manager.task_check(start_url)
        # 启动后一直循环执行
        while True:
            # 每个小时存储一次已抓取数据 或者每天23点存一次
            if datetime.datetime.now().strftime('%M') == '08' or datetime.datetime.now().strftime('%H') == '23':
            ...
            try:
                ...
                if url:
                    task_manager.task_check(url.decode('utf-8'))
                else:
                    time.sleep(2)
...
if __name__ == '__main__':
    ...
    proc_task = Process(target=manager.manager_publish_task, args=('http://www.gmw.cn',))  # 此处填写启动url
