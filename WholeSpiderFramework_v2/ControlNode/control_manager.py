...
class ControlManager(object):
    ...
    def manager_publish_task(self, start_url):
        ...
        while True:
            ...
            elif datetime.datetime.now().strftime('%M') == '00':
    ...
    def _manager_deal_result(self, ch, method, properties, body):
        ...
        if urls:
            if self._manager_msg_count(ch, qname='{}_{}'.format(setting.TASK_NAME, 'task')) >= 10000:
    ...
    @staticmethod
    def _manager_save_data(ch, method, properties, body):
        """
        存储数据
        配置参数：
        ...
        data_manager.data_save_db(data=save_data, dbname='db_public_opinion',
                                  colname='col_' + setting.TASK_NAME)
        ch.basic_ack(delivery_tag=method.delivery_tag)

    def manager_listen_data(self, qname='{}_{}'.format(setting.TASK_NAME, 'data')):
        """
        监听data队列
        :return:
        """
        ch = self._manager_connect_rabbitmq()
        ch.basic_qos(prefetch_count=1)
        ch.basic_consume(self._manager_save_data,
                         queue=qname)
        ch.start_consuming()
...
if __name__ == '__main__':
    ...
    proc_data = Process(target=manager.manager_listen_data)
    proc_task.start()
    proc_result.start()
    proc_data.start()
