...
class Crawl(object):
        ...
        while retry:
            ...
            try:
                ...
                if response.status_code in [200, 404]:
                    res = response.content.decode(encoding=kw.get('pagecode', 'utf-8'), errors='ignore')
                    break
                else:
                    continue
    ...
    def crawl_pic_content(self, url, **kw):
        """
        使用requests使用get方法获取页面content
        :param url:
        :param kw:
        ...
        while retry:
            retry -= 1
            try:
                response = requests.get(url, headers=kw.get('headers'), params=kw.get('params'),
                                            proxies=kw.get('proxies'), timeout=kw.get('timeout', 30))
                if response.status_code in [200, 404]:
